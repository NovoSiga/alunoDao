...
class AlunoDaoMysql {
    ...
    {
        $sql = $this->pdo->prepare("INSERT INTO aluno (nome, turma, curso) VALUES (:nome, :turma, :curso)");
        $sql->bindValue(':nome',$m->getNome());
        $sql->bindValue(':turma', $m->getTurma());
        $sql->bindValue(':curso', $m->getCurso());
        $sql->execute();
    ...
    public function updateAluno (Aluno $u){
        $sql = $this->pdo->prepare("UPDATE aluno SET nome=:nome, turma=:turma, curso=:curso WHERE id=:id");
        $sql->bindValue(':nome',$u->getNome() );
        $sql->bindValue(':turma', $u->getTurma());
        $sql->bindValue(':curso', $u->getCurso());
